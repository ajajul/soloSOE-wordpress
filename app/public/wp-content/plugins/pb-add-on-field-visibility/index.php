<?php
    ...
    */
     ...
     *
     */
    function wppb_fv_update_checker(){
    ...
    add_action( 'plugins_loaded', 'wppb_fv_update_checker', 100 );
     ...
     * @since v.1.0.0
     */
    function wppb_field_visibility_scripts_and_styles($hook) {
...
* @return array
*/
function wppb_field_visibility_get_default_fields() {
    return array(
...
* @return array
*/
function wppb_field_visibility_get_author_meta_fields() {
    return array(
    ...
    function wppb_handle_field_output( $output, $form_location, $field, $user_id, $field_check_errors, $request_data, $input_value = '' ) {
        ...
        $default_get_the_author_fields = wppb_field_visibility_get_author_meta_fields();
    ...
    function wppb_init_field_visibility() {
        $manage_fields = get_option( 'wppb_manage_fields' );
        $filter_fields = wppb_field_visibility_get_extra_fields();
        $filter_default_fields = wppb_field_visibility_get_default_fields();
            ...
            foreach ($manage_fields as $field) {
                foreach ($filter_fields as $filter_field_slug => $filter_field) {
                    if (array_key_exists($filter_field_slug, $filter_default_fields)) {
                        ...
                        add_filter('wppb_register_' . $filter_default_fields[$filter_field_slug], 'wppb_handle_field_output', 10, 6);
                        add_filter('wppb_edit_profile_' . $filter_default_fields[$filter_field_slug], 'wppb_handle_field_output', 10, 6);
                        add_filter('wppb_back_end_' . $filter_default_fields[$filter_field_slug], 'wppb_handle_field_output', 10, 6);
        ...
        foreach( $filter_fields as $filter_field_slug => $filter_field ) {
            if( class_exists('Wordpress_Creation_Kit_PB') ) {
                add_filter('wppb_check_form_field_' . Wordpress_Creation_Kit_PB::wck_generate_slug( $filter_field ), 'wppb_fv_check_field_value', 11, 6);
                add_filter('wppb_check_form_field_' . Wordpress_Creation_Kit_PB::wck_generate_slug( $filter_field ), 'wppb_fv_check_if_user_locked', 11, 6);
            }
        }
