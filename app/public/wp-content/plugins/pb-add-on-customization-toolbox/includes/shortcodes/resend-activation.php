...
function wppb_toolbox_resend_activation_url_handler() {
    ...
    $output .= '
        <li>
        <label for="username_email" style="padding-right: 30px;">'.__( 'E-mail', 'profile-builder' ).'</label>
 ...
 function wppb_toolbox_resend_activation_url() {
 	if(isset($_REQUEST['action']) && $_REQUEST['action']=='wppbc_resend_activation' && isset($_REQUEST['email'])) {
 		global $wpdb;
 		$sql_result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->base_prefix . "signups WHERE user_email = %s", $_REQUEST['email'] ), ARRAY_A );
