...
class CloseTopicControl extends Feature {
    ...
    public function __construct() {
        ...
        if ($this->settings['notify_author']) {
            UserSettings::instance()->register(
                ...
                __("Moderators closed my topic", "gd-bbpress-toolbox"),
                __("Receive instant email notification when moderator or keymaster close your topic.", "gd-bbpress-toolbox"),
                'notifications',
                'checkbox',
                false
            );
    ...
    /** @return bool|\Dev4Press\Plugin\GDBBX\Features\CloseTopicControl */
    ...
    public function send_notifications($topic_id = 0) {
        ...
        $output = gdbbx_module_mailer()->get_topic_author_and_subscribers($topic_id, 'bbp_topic_manual_close_user_ids', $_send_to_author, $_send_to_subscribers, 'topic-closed-notification');
...
Do not reply to this email!", "Email message: notify on topic manual close", "gd-bbpress-toolbox");
        ...
        $subject = _x("[%BLOG_NAME%] The topic '%TOPIC_TITLE%' is now closed", "Email title: notify on topic manual close", "gd-bbpress-toolbox");
