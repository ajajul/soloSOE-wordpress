...
class Report extends Feature {
    ...
    public function script_values($values) {
        ...
        $values['report'] = apply_filters('gdbbx_report_script_values', array(
            'alert' => _x("Report message is required.", "Report Form", "gd-bbpress-toolbox"),
            'after' => _x("Reported", "Report Form", "gd-bbpress-toolbox"),
            'confirm' => _x("Are you sure you want to report this post?", "Report Form", "gd-bbpress-toolbox"),
    ...
    public function notify($user_id, $post_id, $report = '') {
        if ($this->settings['notify_active']) {
            ...
            $start_subject = __($this->settings['notify_subject'], "gd-bbpress-toolbox");
    ...
    private function _string($name) {
        switch ($name) {
            default:
            case 'report':
                return apply_filters('gdbbx_report_string_report', __("Report", "gd-bbpress-toolbox"));
            case 'reported':
                return apply_filters('gdbbx_report_string_reported', __("Reported", "gd-bbpress-toolbox"));
        }
    }
}
