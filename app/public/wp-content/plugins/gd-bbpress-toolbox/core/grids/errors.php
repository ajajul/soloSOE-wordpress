...
class gdbbx_grid_errors extends d4p_grid {
    ...
    public function rows_per_page() {
        ...
        $per_page = get_user_meta($user, 'gdbbx_rows_per_page_errors', true);
    ...
    public function get_columns() {
        return array(
            ...
            'message' => __("Message", "gd-bbpress-toolbox"),
    ...
    public function column_file($item) {
        $actions = array(
            'delete' => sprintf('<a href="admin.php?page=gd-bbpress-toolbox-errors&single-action=%s&error=%s&gdbbx_handler=getback&_wpnonce=%s">%s</a>', 'delete', $item->meta_id, wp_create_nonce('gd-bbpress-toolbox-error'), __("Delete", "gd-bbpress-toolbox"))
        );
    ...
    public function column_topic($item) {
        ...
        if ($item->post_type == 'reply') {
            $title .= __("Reply", "gd-bbpress-toolbox").': <strong>'.$item->ID.'</strong><br/>';
            $title .= bbp_get_reply_title_fallback(bbp_get_reply_title($item->ID), $item->ID);
            $url = bbp_get_reply_url($item->ID);
        } else {
            $title .= __("Topic", "gd-bbpress-toolbox").': <strong>'.$item->ID.'</strong><br/>';
            $title .= bbp_get_topic_title($topic_id);
            $url = get_permalink($topic_id);
        }

        $actions = array(
            'narrow' => sprintf('<a href="admin.php?page=gd-bbpress-toolbox-errors&bbp_topic_id=%s">%s</a>', $topic_id, __("Filter", "gd-bbpress-toolbox")),
            'visit' => sprintf('<a href="%s">%s</a>', $url, __("Visit", "gd-bbpress-toolbox")),
            'edit' => sprintf('<a href="post.php?post=%s&action=edit">%s</a>', $item->ID, __("Edit", "gd-bbpress-toolbox")),
        );
