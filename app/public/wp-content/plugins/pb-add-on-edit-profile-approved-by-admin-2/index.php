<?php
/*
Plugin Name: Profile Builder - Edit Profile Approved by Admin Add-On
Plugin URI: http://www.cozmoslabs.com/wordpress-profile-builder/
Description: Extends the functionality of Profile Builder by allowing administrators to approve edit profile changes by users
Version: 1.0.7
 ...
 *
 */
define('WPPBEPAA_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('WPPBEPAA_PLUGIN_URL', plugin_dir_url(__FILE__));
 ...
 *
 */
function wppb_epaa_update_checker(){
    if( class_exists('wppb_PluginUpdateChecker') ) {
        ...
        $wppb_epaa_update = new wppb_PluginUpdateChecker('http://updatemetadata.cozmoslabs.com/?localSerialNumber=' . $localSerial . '&uniqueproduct=CLPBEPAA', __FILE__, 'wppb-epaa-add-on');
    }
}
add_action( 'plugins_loaded', 'wppb_epaa_update_checker', 100 );
...
*
*/
add_action( 'init', 'wppb_init_edit_profile_approval' );
function wppb_init_edit_profile_approval() {

    if( !class_exists('Wordpress_Creation_Kit_PB') )
        return;

    //handel fields visual output
    add_filter( 'wppb_extra_attribute', 'wppb_epaa_handle_field_visual_output', 20 , 3 );
    if( !current_user_can( 'manage_options' ) || !current_user_can( 'edit_users' ) ) {
        add_filter('wppb_output_after_form_field', 'wppb_epaa_handle_field_approval_message', 19, 5);
    }

    if( current_user_can( 'manage_options' ) || current_user_can( 'edit_users' ) ) {
        add_filter('wppb_output_after_form_field', 'wppb_epaa_handle_field_approval_button', 20, 5);
        add_filter('wppb_after_form_fields', 'wppb_epaa_buttons_for_admins', 20, 3 );
    }

    //handle field save
    add_filter( 'wppb_build_userdata', 'wppb_epaa_handle_default_fields_save', 99, 3 );
    add_filter( 'wppb_pre_save_form_field', 'wppb_epaa_handle_fields_with_meta_save', 1 , 5 );

    //handle what value is displayed
    add_filter( "get_user_metadata", 'wppb_epaa_maybe_show_alternative_value', 10, 4 );
    add_filter( "wppb_user_meta_exists_meta_name", 'wppb_epaa_check_alternative_value_exists', 20, 3 );
    add_filter( 'get_the_author_user_email', 'wppb_epaa_maybe_show_alternative_value_for_user_email', 99, 3 );
    add_filter( 'get_the_author_user_url', 'wppb_epaa_maybe_show_alternative_value_for_user_url', 99, 3 );

    //handle notification to admin
    add_action( 'wppb_edit_profile_success', 'wppb_epaa_send_notification_to_admin', 10, 3 );

    //show edit profile notification to user
    add_action( 'wppb_edit_profile_success', 'wppb_epaa_show_edit_profile_notification', 10, 3 );

    //add a meta when a user edits his profile and has unapproved fields
    add_action( 'wppb_edit_profile_success', 'wppb_epaa_add_awaiting_review_meta', 10, 3 );
    ...
    if( ( !is_multisite() && current_user_can( 'edit_users' ) ) || ( is_multisite() && current_user_can( 'manage_network' ) ) ) {
        ...
        add_filter( 'wppb_edit_other_users_dropdown_query_args', 'wppb_epaa_show_in_dropdown_only_users_with_unapproved_fields' );
        //show links on edit profile form above the edit other users dropdown
        add_action('wppb_before_edit_profile_fields', 'wppb_epaa_approval_mode_links_for_admin', 5);

        if (isset($_REQUEST['wppb_epaa_review_users']) && $_REQUEST['wppb_epaa_review_users'] == 'true') {
            //make sure we show the dropdown
            add_filter('wppb_display_edit_other_users_dropdown', 'wppb_epaa_force_display_edit_other_user_dropdown');
            add_filter('wppb_edit_other_users_count_limit', 'wppb_epaa_force_edit_other_users_count_limit');
            //force show all fields on the edit profile form
            remove_filter('wppb_change_form_fields', 'wppb_multiple_forms_change_fields', 10, 2);
        }
    }
...
//set up ajax hooks
add_action( 'wp_ajax_wppb_epaa_approve_values', 'wppb_epaa_approve_values' );
/**
 * Function that approves the values and sends the notifications
 */
function wppb_epaa_approve_values(){
    if( current_user_can( 'manage_options' ) || current_user_can( 'edit_users' ) ){
        $field_ids = $_POST['fieldIDS']; //should be a comma separated list of ids
        if( !empty( $field_ids ) )
            $field_ids = explode(',', $field_ids);

        if( !empty( $_POST['unapprovedIDS'] ) )
            $unapproved_field_ids = $_POST['unapprovedIDS']; //should be a comma separated list of ids
        else
                ...
                foreach ( $all_fields as $field ){
                    if( $field['field'] != 'Repeater' ) {
                        if (in_array($field['id'], $field_ids)) {
                            ...
                            $meta_name = wppb_epaa_get_alternative_meta_for_field($field);
                            $alternative_meta_name = wppb_epaa_prefix_alternative_meta_name($meta_name);
                            ...
                            for ( $i = 0; $i <= $extra_groups_count; $i++ ){
                                ...
                                foreach( $indexed_repeater_group as $rp_field ){
                                    if (in_array($rp_field['id'], $field_ids)) {
                                        $meta_name = wppb_epaa_get_alternative_meta_for_field($rp_field);
                                        $alternative_meta_name = wppb_epaa_prefix_alternative_meta_name($meta_name);
                                        $alternative_meta_value = get_user_meta($user_id, $alternative_meta_name, true);
                                        delete_user_meta($user_id, $alternative_meta_name);
                                        update_user_meta($user_id, $meta_name, $alternative_meta_value);
        ...
        wppb_epaa_send_notification_to_user( $user_id, $field_ids, $unapproved_field_ids );
 ...
 */
function wppb_epaa_properties_manage_field( $fields ) {
...
add_filter( 'wppb_manage_fields', 'wppb_epaa_properties_manage_field' );
 ...
 * @since v.1.0.0
 */
function wppb_epaa_scripts_and_styles($hook) {
    if ( $hook == 'profile-builder_page_manage-fields' ) {
        wp_enqueue_script('wppb-epaa-js', plugin_dir_url(__FILE__) . 'assets/js/main.js', array('jquery', 'wppb-manage-fields-live-change'));
    }
    wp_enqueue_script( 'wppb-epaa-frontend-js', plugin_dir_url(__FILE__) . 'assets/js/epaa-frontend.js', array( 'jquery' ) );
...
add_action( 'admin_enqueue_scripts', 'wppb_epaa_scripts_and_styles' );
add_action( 'wp_enqueue_scripts', 'wppb_epaa_scripts_and_styles' );
...
* @return array
*/
function wppb_epaa_default_fields_with_no_meta() {
    return array(
 ...
 * @return mixed
 */
function wppb_epaa_get_alternative_meta_for_field( $field ){
    $filter_default_fields = wppb_epaa_default_fields_with_no_meta();
 ...
 * @return string
 */
function wppb_epaa_prefix_alternative_meta_name( $meta_name ){
 ...
 */
function wppb_epaa_get_desired_user_id( ){
 ...
 *
 */
function wppb_change_display_value_to_icon_epaa( $display_value ) {
...
add_filter( 'wck_pre_displayed_value_wppb_manage_fields_element_edit-profile-approved-by-admin', 'wppb_change_display_value_to_icon_epaa' );
 ...
 * @return string
 */
function wppb_epaa_buttons_for_admins( $content, $form_location, $form_id ){
    if( $form_location === 'edit_profile' ){

        $found = false;

        $user_id = wppb_epaa_get_desired_user_id();
...
function wppb_epaa_handle_field_approval_message( $output, $field, $form_id, $form_location, $called_from ){
    if( $form_location === 'edit_profile' ){
        if( isset( $field['edit-profile-approved-by-admin'] ) && $field['edit-profile-approved-by-admin'] === 'yes' ) {
 ...
 * @return string
 */
function wppb_epaa_handle_field_approval_button( $output, $field, $form_id, $form_location, $called_from ){
    if( $form_location === 'edit_profile' ){
        if( isset( $field['edit-profile-approved-by-admin'] ) && $field['edit-profile-approved-by-admin'] === 'yes' ) {
 ...
 * @return string
 */
function wppb_epaa_handle_field_visual_output( $extra_attributes, $field, $form_location ){
 ...
 * @return mixed
 */
function wppb_epaa_handle_default_fields_save( $userdata, $global_request, $form_args ){
    if( $form_args['form_type'] === 'edit_profile' ){
        if ( !current_user_can('manage_options') ) {
            if (!empty($form_args['form_fields'])) {
                ...
                remove_filter( "get_user_metadata", 'wppb_epaa_maybe_show_alternative_value', 10, 4 );
 ...
 * @return bool
 */
function wppb_epaa_handle_fields_with_meta_save( $bool, $field, $user_id, $request_data, $form_location ){
        ...
        if ( !current_user_can('manage_options') ) {
            ...
            if( $field['field'] == 'Repeater' ){//handle repeater
                ...
                for ( $i = 0; $i <= $extra_groups_count; $i++ ){
                    ...
                    foreach( $indexed_repeater_group as $rp_field ){
                        wppb_epaa_handle_fields_with_meta_save( $bool, $rp_field, $user_id, $request_data, $form_location );
                    }
                }
                    ...
                    if (!empty($field['meta-name'])) {
                        ...
                        if (isset($new_value)) {
                            ...
                            if ($old_meta_value != $new_value) { //the default fields with meta won't pass this as they save higher in the code
                                update_user_meta($user_id, wppb_epaa_prefix_alternative_meta_name($field['meta-name']), $new_value);
 ...
 * @return array
 */
function wppb_epaa_maybe_show_alternative_value( $replace, $object_id, $meta_key, $single){
    ...
    if ( current_user_can('manage_options') || $object_id === get_current_user_id() ) {
        ...
        $unapproved_meta_key = wppb_epaa_prefix_alternative_meta_name($meta_key);
 ...
 * @return mixed
 */
function wppb_epaa_maybe_show_alternative_value_for_user_url( $value, $user_id, $original_user_id ){
    //this applies only to logged in users specifically the admins and the user that is editing his profile
    if( !is_user_logged_in() )
        return $value;

    if ( current_user_can('manage_options') || $user_id === get_current_user_id() ) {
        $unapproved_value_exists = wppb_user_meta_exists($user_id, wppb_epaa_prefix_alternative_meta_name('website') );
        if (!empty($unapproved_value_exists)) {
            $alternative_value = get_user_meta( $user_id, wppb_epaa_prefix_alternative_meta_name('website'), true );
 ...
 * @return mixed
 */
function wppb_epaa_maybe_show_alternative_value_for_user_email( $value, $user_id, $original_user_id ){
    //this applies only to logged in users specifically the admins and the user that is editing his profile
    if( !is_user_logged_in() )
        return $value;

    if ( current_user_can('manage_options') || $user_id === get_current_user_id() ) {
        $unapproved_value_exists = wppb_user_meta_exists($user_id, wppb_epaa_prefix_alternative_meta_name('email') );
        if (!empty($unapproved_value_exists)) {
            $alternative_value = get_user_meta( $user_id, wppb_epaa_prefix_alternative_meta_name('email'), true );
            return $alternative_value;
        }
    }
 ...
 * @param $user_id
 */
function wppb_epaa_send_notification_to_user( $user_id, $field_ids, $unapproved_field_ids ){
 ...
 * @param $user_id
 */
function wppb_epaa_send_notification_to_admin( $request, $form_name, $user_id ){
    global $wppb_epaa_fields_awaiting_approval;
    if( function_exists('wppb_mail') && !empty( $wppb_epaa_fields_awaiting_approval ) && apply_filters( 'wppb_epaa_send_notification_to_admin', true ) ){
        ...
        $approve_url = '<a href="'. esc_url_raw( add_query_arg( array( 'edit_user' => $user_id, 'wppb_epaa_review_users' => 'true' ), wppb_epaa_determine_url_from_form_name($form_name) ) ) .'" target="_blank">here</a>';
 ...
 * @param $user_id
 */
function wppb_epaa_show_edit_profile_notification( $request, $form_name, $user_id ){
    global $wppb_epaa_fields_awaiting_approval;
 ...
 * @return false|string
 */
function wppb_epaa_determine_url_from_form_name( $form_name ){
    if( $form_name != 'unspecified' ){
        ...
        $default_form_pages = array_udiff( $all_edit_form_pages, $specific_form_pages, 'wppb_epaa_compare_objects' );
 ...
 * @return mixed
 */
function wppb_epaa_compare_objects($obj_a, $obj_b) {
...
function wppb_epaa_check_alternative_value_exists( $result, $id, $meta_name ){
...
function wppb_epaa_force_display_edit_other_user_dropdown($bool){
    return true;
}

//increase the number of the limit of users so we always show the edit other users dropdown
function wppb_epaa_force_edit_other_users_count_limit($number){
    return 1000000;
}
 ...
 */
function wppb_epaa_approval_mode_links_for_admin(){
    echo '<p id="wppb-epaa-admin-review-links">';
    echo '<a href="'. esc_url_raw( add_query_arg( 'wppb_epaa_review_users', 'true', remove_query_arg( 'wppb_epaa_reviewed_users_with_unapproved_fields', wppb_curpageurl() ) ) ) .'">'. __( 'Show users that require review', 'profile-builder' ) .'</a>';
    echo ' | ';
    echo '<a href="'. esc_url_raw( add_query_arg( array( 'wppb_epaa_review_users' => 'true', 'wppb_epaa_reviewed_users_with_unapproved_fields' => 'true' ), wppb_curpageurl() ) ).'">'. __( 'Show reviewed users with unapproved fields', 'profile-builder' ) .'</a>';
    if( isset( $_REQUEST['wppb_epaa_review_users'] ) && $_REQUEST['wppb_epaa_review_users'] == 'true' ) {
        echo ' | ';
        echo '<a href="'. esc_url_raw( remove_query_arg( array( 'wppb_epaa_review_users', 'wppb_epaa_reviewed_users_with_unapproved_fields' ), wppb_curpageurl() ) ).'">'. __( 'Exit Review Mode', 'profile-builder' ) .'</a>';
    }
    echo '</p>';
}
 ...
 * @param $user_id
 */
function wppb_epaa_add_awaiting_review_meta( $request, $form_name, $user_id ){
    global $wppb_epaa_fields_awaiting_approval;
    if( !empty( $wppb_epaa_fields_awaiting_approval ) ){
 ...
 * @return mixed
 */
function wppb_epaa_show_in_dropdown_only_users_with_unapproved_fields( $query_args ){
